...
public class DemoMultiArrays {
	...
	/* This MultiArray is a "wrapper" around the Java
           multidimensional array sharing the same storage for values,
           so setting a value of this MultiArray will change the
           underlying value. */
	ma.setDouble(new int[] {0,0,0}, -99.0);
//	assert (dd[0][0][0] == -99, "should have changed dd[0][0][0]!");
	/* Restore value so it's easy to recognize element indices from
           values in subsequent operations */
	ma.setDouble(new int[] {0,0,0}, 0.);

	/* A MultiArrayProxy is a MultiArray that is a "view" of another
           underlying MultiArray to make it look clipped or flipped or
           transposed or sliced or ....  MultiArrayProxys involve no
           copying of values; the underlying storage is still shared.*/

	/* Create a MultiArrayProxy that behaves as if the values are
           flipped along the zeroth dimension.  */
	...
	/* Flip it along the zeroth dimension and then flip the result
           along the second dimension */
	maFlipped = new MultiArrayProxy(ma, new FlipMap(new FlipMap(0), 2));
	System.out.println("maFlipped = " + MultiArrayToString(maFlipped));

	/* Clip the 3 x 4 x 5 MultiArray along the zeroth dimension to
	...
	/* Clip it along the first dimension to make a 3 x 2 x 5
           MultiArray */
	maClipped = new MultiArrayProxy(ma, new ClipMap(1, 1, 2));
	System.out.println("maClipped = " + MultiArrayToString(maClipped));
	
	/* Clip it along the second dimension to make a 3 x 4 x 2
           MultiArray */
	...
	/* Clip along the zeroth and then second dimension to make a 2 x 4 x 2
           MultiArray */
	maClipped = new MultiArrayProxy(ma, new ClipMap(new ClipMap(0, 1, 2), 
							2, 1, 2));
	System.out.println("maClipped = " + MultiArrayToString(maClipped));
	
	/* Clip along the second and then zeroth dimension to make a 2 x 4 x 2
           MultiArray */
	maClipped = new MultiArrayProxy(ma, new ClipMap(new ClipMap(2, 1, 2), 
							0, 1, 2));
	System.out.println("maClipped = " + MultiArrayToString(maClipped));
	
	boolean[] everyOther = new boolean[] {true, false};
	/* Decimate along the zeroth dimension to take every other slab
           to make a 2 x 2 x 4 MultiArray */
	MultiArray maDecimated = new MultiArrayProxy(ma, 
					       new DecimateMap(0, 
							       everyOther));
	System.out.println("maDecimated = " + MultiArrayToString(maDecimated));
	
	/* Decimate along the first dimension taking every other slab
           to make a 3 x 1 x 4 MultiArray */
	maDecimated = new MultiArrayProxy(ma, 
				    new DecimateMap(1, 
						    everyOther));
	System.out.println("maDecimated = " + MultiArrayToString(maDecimated));
	
	/* Decimate along the second dimension taking every other value
           to make a 3 x 2 x 2 MultiArray */
	maDecimated = new MultiArrayProxy(ma, 
				    new DecimateMap(2, 
						    everyOther));
	System.out.println("maDecimated = " + MultiArrayToString(maDecimated));
	
	/* Decimate along the first and then second dimensions taking
           every other value to make a 3 x 1 x 2 MultiArray */
	maDecimated = new MultiArrayProxy(ma, 
				    new DecimateMap(new DecimateMap(1, 
								    everyOther),
						    2, 
						    everyOther));
	System.out.println("maDecimated = " + MultiArrayToString(maDecimated));
	
	/* Flatten it along the zeroth dimension to make a 12 x 5
           MultiArray */
	MultiArray maFlattened = new MultiArrayProxy(ma, 
						     new FlattenMap(0));
	System.out.println("maFlattened = " + MultiArrayToString(maFlattened));
	
	/* Flatten it along the first dimension to make a 3 x 20
           MultiArray */
	maFlattened = new MultiArrayProxy(ma, new FlattenMap(1));
	System.out.println("maFlattened = " + MultiArrayToString(maFlattened));
	
	/* Flatten along the zeroth dimension and then the zeroth
           dimension of the result to make a 60-element MultiArray */
	maFlattened = new MultiArrayProxy(ma, 
					  new FlattenMap(
					      new FlattenMap(0), 0));
	System.out.println("maFlattened = " + MultiArrayToString(maFlattened));

	/* Slice it along the zeroth dimension at index 1 to make a 4 x 5
           MultiArray */
	MultiArray maSliced = new MultiArrayProxy(ma, new SliceMap(0, 1));
	System.out.println("maSliced = " + MultiArrayToString(maSliced));

	/* Slice it along the first dimension at index 1 to make a 3 x 5
           MultiArray */
	maSliced = new MultiArrayProxy(ma, new SliceMap(1, 1));
	System.out.println("maSliced = " + MultiArrayToString(maSliced));

	/* Slice it along the second dimension at index 1 to make a 3 x 4
           MultiArray */
	maSliced = new MultiArrayProxy(ma, new SliceMap(2, 1));
	System.out.println("maSliced = " + MultiArrayToString(maSliced));

	/* Slice it along the zeroth dimension at index 1, then the
           result along the first dimension at index 1 to make a
           1-dimensional MultiArray of length 4 */
	maSliced = new MultiArrayProxy(ma, new SliceMap(new SliceMap(0, 1), 
							1, 1));
	System.out.println("maSliced = " + MultiArrayToString(maSliced));

	/* Get the [2][1][3] element by slicing 3 times, along the
           zeroth dimension at index 2, then the result along its zeroth
           dimension at index 1, then the result along its zeroth
           dimension at index 3 to yield a scalar MultiArray.  Note we
           can get this element a lot easier with just 
	      ma.getDouble(new int[] {2,1,3})
	*/
	maSliced = new MultiArrayProxy(ma, 
				       new SliceMap(
					   new SliceMap(
					       new SliceMap(
						   0, 2), 
	...
	/* Transpose it by swapping the zeroth and second dimensions to
           make a 5 x 4 x 3 MultiArray */
	MultiArray maTransposed = new MultiArrayProxy(ma, 
						      new TransposeMap(0, 2));
	System.out.println("maTransposed = " + MultiArrayToString(maTransposed));

	/* Flip along the first dimension, then clip it along the zeroth
            dimension to make a 2 x 4 x 5 MultiArray */
	MultiArray maClipFlip = new MultiArrayProxy(ma, 
						    new ClipMap(
							new FlipMap(1),
							0, 1, 2));
