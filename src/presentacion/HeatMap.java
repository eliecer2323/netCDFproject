...
public class HeatMap extends JPanel {
	...
	/**
	 * Specify the coordinate bounds for the Y Max. Only used for the axis labels,
	...
	/**
	 * Updates the gradient used to display the data. Calls drawData() and repaint()
	 * when finished.
	 * 
	...
	/**
	 * This uses the current array of colors that make up the gradient, and assigns
	 * a color index to each data point, stored in the dataColorIndices array, which
	...
	/**
	 * This function generates data that is not vertically-symmetric, which makes it
	 * very useful for testing which type of vertical axis is being used to plot the
	 * data. If the graphics Y-axis is used, then the lowest values should be
	 * displayed at the top of the frame. If the non-graphics (mathematical
	 * coordinate-system) Y-axis is used, then the lowest values should be displayed
	...
	/**
	 * This function generates an appropriate data array for display. It uses the
	 * function: z = Math.cos(Math.abs(sX) + Math.abs(sY)). The parameter specifies
	 * the number of data points in each direction, producing a square matrix.
	 * 
	 * @param dimension Size of each side of the returned array
	...
	/**
	 * Updates the data display, calls drawData() to do the expensive re-drawing of
	 * the data plot, and then calls repaint().
	 * 
	 * @param data             The data to display, must be a complete array
	 *                         (non-ragged)
	 * @param useGraphicsYAxis If true, the data will be displayed with the y=0 row
	 *                         at the top of the screen. If false, the data will be
	 *                         displayed with the y=0 row at the bottom of the
	 *                         screen.
	 */
	 ...
	 * was spend drawing the actual data (not on the axes or tick marks). Since the
	 * Graphics2D has a drawImage method that can do scaling, we are using that
	 * instead of scaling it ourselves. We only need to draw the data into the
	 * bufferedImage on startup, or if the data or gradient changes. This saves us
	 * an enormous amount of time. Thanks to Josh Hayes-Sheen (grey@grevian.org) for
	 * the suggestion and initial code to use the BufferedImage technique.
	 * 
	 ...
	 * pixel per data point. Too bad there isn't a setPixel method in the Graphics2D
	 * class, it seems a bit silly to fill a rectangle just to set a single pixel...
	 *
	...
	/**
	 * The overridden painting method, now optimized to simply draw the data plot to
	 * the screen, letting the drawImage method do the resizing. This saves an
	...
	public void paintComponent(Graphics g) {
		...
		// axis ticks - ticks start even with the bottom left coner, end very close to
		// end of line (might not be right on)
